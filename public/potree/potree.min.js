...
return''}(),maxNodesLoadGPUFrame:20,maxDEMLevel:0,maxNodesLoading:void 0!==navigator.hardwareConcurrency?navigator.hardwareConcurrency:4,pointLoadLimit:1E10,numNodesLoading:0,measureTimings:!1,workerPool:new y,lru:new oa,pointcloudTransformVersion:void 0,denormalizeUint8Array:function(a){const b=new Float32Array(a.length);for(let c=0;c<b.length;c++)b[c]=a[c]/255;return b}},x={POSITION_CARTESIAN:0,COLOR_PACKED:1,COLOR_FLOATS_1:2,COLOR_FLOATS_255:3,NORMAL_FLOATS:4,FILLER:5,INTENSITY:6,CLASSIFICATION:7,
NORMAL_SPHEREMAPPED:8,NORMAL_OCT16:9,NORMAL:10,RETURN_NUMBER:11,NUMBER_OF_RETURNS:12,SOURCE_ID:13,INDICES:14,SPACING:15},C={DATA_TYPE_DOUBLE:{ordinal:0,size:8},DATA_TYPE_FLOAT:{ordinal:1,size:4},DATA_TYPE_INT8:{ordinal:2,size:1},DATA_TYPE_UINT8:{ordinal:3,size:1},DATA_TYPE_INT16:{ordinal:4,size:2},DATA_TYPE_UINT16:{ordinal:5,size:2},DATA_TYPE_INT32:{ordinal:6,size:4},DATA_TYPE_UINT32:{ordinal:7,size:4},DATA_TYPE_INT64:{ordinal:8,size:8},DATA_TYPE_UINT64:{ordinal:9,size:8}},pa=0,qa;for(qa in C)C[pa]=
C[qa],pa++;t.POSITION_CARTESIAN=new t(x.POSITION_CARTESIAN,C.DATA_TYPE_FLOAT,3);t.RGBA_PACKED=new t(x.COLOR_PACKED,C.DATA_TYPE_INT8,4);t.COLOR_PACKED=t.RGBA_PACKED;t.RGB_PACKED=new t(x.COLOR_PACKED,C.DATA_TYPE_INT8,3);t.NORMAL_FLOATS=new t(x.NORMAL_FLOATS,C.DATA_TYPE_FLOAT,3);t.FILLER_1B=new t(x.FILLER,C.DATA_TYPE_UINT8,1);t.INTENSITY=new t(x.INTENSITY,C.DATA_TYPE_UINT16,1);t.CLASSIFICATION=new t(x.CLASSIFICATION,C.DATA_TYPE_UINT8,1);t.NORMAL_SPHEREMAPPED=new t(x.NORMAL_SPHEREMAPPED,C.DATA_TYPE_UINT8,
2);t.NORMAL_OCT16=new t(x.NORMAL_OCT16,C.DATA_TYPE_UINT8,2);t.NORMAL=new t(x.NORMAL,C.DATA_TYPE_FLOAT,3);t.RETURN_NUMBER=new t(x.RETURN_NUMBER,C.DATA_TYPE_UINT8,1);t.NUMBER_OF_RETURNS=new t(x.NUMBER_OF_RETURNS,C.DATA_TYPE_UINT8,1);t.SOURCE_ID=new t(x.SOURCE_ID,C.DATA_TYPE_UINT8,1);t.INDICES=new t(x.INDICES,C.DATA_TYPE_UINT32,1);t.SPACING=new t(x.SPACING,C.DATA_TYPE_FLOAT,1);F.prototype.add=function(a){this.attributes.push(a);this.byteSize+=a.byteSize;this.size++};F.prototype.hasColors=function(){for(var a in this.attributes)if(this.attributes[a].name===
x.COLOR_PACKED)return!0;return!1};F.prototype.hasNormals=function(){for(var a in this.attributes){var b=this.attributes[a];if(b===t.NORMAL_SPHEREMAPPED||b===t.NORMAL_FLOATS||b===t.NORMAL||b===t.NORMAL_OCT16)return!0}return!1};class fa{constructor(a,b,c){this.name=a;this.box=b;this.tileSize=c;this.level=this.name.length-1;this.data=new Float32Array(c*c);this.data.fill(-Infinity);this.children=[];this.mipMap=[this.data];this.mipMapNeedsUpdate=!0}createMipMap(){this.mipMap=[this.data];for(var a=this.tileSize,
b=parseInt(a/2),c=this.data;1<b;){for(var e=new Float32Array(b*b),d=0;d<b;d++)for(var f=0;f<b;f++){var g=c[2*d+2*f*a],k=c[2*d+2*f*a+a],h=c[2*d+1+2*f*a],l=c[2*d+1+2*f*a+a],[m,p]=[0,0];isFinite(g)&&(m+=g,p+=1);isFinite(k)&&(m+=k,p+=1);isFinite(h)&&(m+=h,p+=1);isFinite(l)&&(m+=l,p+=1);m/=p;e[d+f*b]=m}this.mipMap.push(e);c=e;a=b;b=parseInt(b/2)}this.mipMapNeedsUpdate=!1}uv(a){var b=this.box.size(new THREE.Vector3);return[(a.x-this.box.min.x)/b.x,(a.y-this.box.min.y)/b.y]}heightAtMipMapLevel(a,b){a=this.uv(a);
var c=parseInt(this.tileSize/parseInt(2**b)),e=this.mipMap[b];b=Math.min(a[0]*c,c-1);a=Math.min(a[1]*c,c-1);var d=b%1,f=a%1,[g,k]=[Math.floor(b),Math.ceil(b)],[h,l]=[Math.floor(a),Math.ceil(a)];b=e[g+c*h];a=e[g+c*l];var m=e[k+c*h];c=e[k+c*l];e=isFinite(b)?(1-d)*(1-f):0;var p=isFinite(a)?(1-d)*f:0,n=isFinite(m)?d*(1-f):0;d=isFinite(c)?d*f:0;f=e+p+n+d;e/=f;p/=f;n/=f;d/=f;if(0===f)return null;f=0;isFinite(b)&&(f+=b*e);isFinite(a)&&(f+=a*p);isFinite(m)&&(f+=m*n);isFinite(c)&&(f+=c*d);return f}height(a){for(var b=
null,c=0;c<this.mipMap.length&&(b=this.heightAtMipMapLevel(a,c),null===b);c++);return b}traverse(a,b=0){a(this,b);for(var c of this.children.filter(a=>void 0!==a))c.traverse(a,b+1)}}class ra{constructor(a){this.pointcloud=a;this.boundingBox=this.matrix=null;this.tileSize=64;this.root=null;this.version=0}expandAndFindByBox(a,b){if(0===b)return[this.root];for(var c=[],e=[this.root];0<e.length;){var d=e.pop(),f=d.box.size(new THREE.Vector3),g=(a.min.x-d.box.min.x)/f.x,k=(a.min.y-d.box.min.y)/f.y,h=(a.max.x-
d.box.max.x)/f.x,l=(a.max.y-d.box.max.y)/f.y;g=.5>g?0:1;k=.5>k?0:1;h=.5>h?0:1;l=.5>l?0:1;g=0===g&&0===k&&1===h&&1===l?[0,1,2,3]:g===h&&k===l?[g<<1|k]:[g<<1|k,h<<1|l];for(var m of g)void 0===d.children[m]&&(g=d.box.clone(),0<(m&2)?g.min.x+=f.x/2:g.max.x-=f.x/2,0<(m&1)?g.min.y+=f.y/2:g.max.y-=f.y/2,g=new fa(d.name+m,g,this.tileSize),d.children[m]=g),g=d.children[m],g.level<b?e.push(g):c.push(g)}return c}childIndex(a){var [b,c]=a.map(a=>.5>a?0:1);return b<<1|c}height(a){if(!this.root)return 0;for(var b=
null,c=[this.root];;){var e=c[c.length-1],d=e.height(a);null!==d&&(b=d);d=e.uv(a);d=this.childIndex(d);if(e.children[d])c.push(e.children[d]);else break}return b+this.pointcloud.position.z}update(a){null!==this.matrix&&this.matrix.equals(this.pointcloud.matrixWorld)||(this.matrix=this.pointcloud.matrixWorld.clone(),this.boundingBox=this.pointcloud.boundingBox.clone().applyMatrix4(this.matrix),this.root=new fa('r',this.boundingBox,this.tileSize),this.version++);var b=null,c;for(c of a)if(void 0===
c.demVersion||c.demVersion<this.version){b=c;break}if(null!==b){var e=b.getBoundingBox().clone().applyMatrix4(this.matrix),d=e.size(new THREE.Vector3),f=this.expandAndFindByBox(e,b.getLevel());b.demVersion=this.version;a=b.geometryNode.geometry.attributes.position.array;b={boundingBox:{min:b.getBoundingBox().min.toArray(),max:b.getBoundingBox().max.toArray()},position:(new Float32Array(a)).buffer};var g=this;u.workerPool.runTask(y.DEM,function(a){a=new Float32Array(a.data.dem.data);for(var b of f){for(var c=
b.box.size(new THREE.Vector3),k=0;k<g.tileSize;k++)for(var p=0;p<g.tileSize;p++){var n=g.tileSize*(b.box.min.x+k/(g.tileSize-1)*c.x-e.min.x)/d.x,q=g.tileSize*(b.box.min.y+p/(g.tileSize-1)*c.y-e.min.y)/d.y;0>n||n>g.tileSize||0>q||q>g.tileSize||(n=Math.min(Math.floor(n),g.tileSize-1),q=Math.min(Math.floor(q),g.tileSize-1),b.data[k+g.tileSize*p]=a[n+g.tileSize*q])}b.createMipMap();b.mipMapNeedsUpdate=!0}},b,[b.position])}}}class V{constructor(){this.needsTransformUpdate=!0}getChildren(){}getBoundingBox(){}isLoaded(){}isGeometryNode(){}isTreeNode(){}getLevel(){}getBoundingSphere(){}}
class S extends THREE.Object3D{constructor(){super();this.dem=new ra(this)}initialized(){return null!==this.root}}class sa{constructor(){this.spacing=0;this.nodes=this.root=this.boundingBox=null;this.pointAttributes={};this.hierarchyStepSize=-1;this.serverURL=this.boundingSphere=this.projection=this.offset=this.baseDepth=this.schema=this.loader=null;this.normalize={color:!1,intensity:!1}}}z.IDCount=0;z.prototype=Object.create(V.prototype);z.prototype.isGeometryNode=function(){return!0};z.prototype.isTreeNode=
function(){return!1};z.prototype.isLoaded=function(){return this.loaded};z.prototype.getBoundingSphere=function(){return this.boundingSphere};z.prototype.getBoundingBox=function(){return this.boundingBox};z.prototype.getLevel=function(){return this.level};z.prototype.getChildren=function(){for(var a=[],b=0;8>b;++b)this.children[b]&&a.push(this.children[b]);return a};z.prototype.getURL=function(){var a=this.greyhoundBounds;a=''+this.pcoGeometry.serverURL+'read?depthBegin='+(this.baseLoaded?this.level+
this.pcoGeometry.baseDepth:0)+'&depthEnd='+(this.level+this.pcoGeometry.baseDepth+1)+'&bounds=['+(a.min.x+','+a.min.y+','+a.min.z+','+a.max.x+','+a.max.y+','+a.max.z)+']&schema='+JSON.stringify(this.pcoGeometry.schema)+'&compress=true';this.scale&&(a+='&scale='+this.scale);if(this.greyhoundOffset){var b=this.greyhoundOffset;a+='&offset=['+b.x+','+b.y+','+b.z+']'}this.baseLoaded||(this.baseLoaded=!0);return a};z.prototype.addChild=function(a){this.children[a.index]=a;a.parent=this};z.prototype.load=
function(){!0===this.loading||!0===this.loaded||u.numNodesLoading>=u.maxNodesLoading||(this.loading=!0,u.numNodesLoading++,0===this.level%this.pcoGeometry.hierarchyStepSize&&this.hasChildren?this.loadHierarchyThenPoints():this.loadPoints())};z.prototype.loadPoints=function(){this.pcoGeometry.loader.load(this)};z.prototype.loadHierarchyThenPoints=function(){var a=[0,2,1,3,4,6,5,7],b=function(b){var c=0;Object.keys(b).forEach(function(b){'swd'===b?c+=1<<a[0]:'nwd'===b?c+=1<<a[1]:'swu'===b?c+=1<<a[2]:
'nwu'===b?c+=1<<a[3]:'sed'===b?c+=1<<a[4]:'ned'===b?c+=1<<a[5]:'seu'===b?c+=1<<a[6]:'neu'===b&&(c+=1<<a[7])});return c},c=function(d,e,f){var g,h;Object.keys(d).forEach(function(k){if('n'!==k){switch(k){case 'swd':g=d.swd;h=e+a[0];break;case 'nwd':g=d.nwd;h=e+a[1];break;case 'swu':g=d.swu;h=e+a[2];break;case 'nwu':g=d.nwu;h=e+a[3];break;case 'sed':g=d.sed;h=e+a[4];break;case 'ned':g=d.ned;h=e+a[5];break;case 'seu':g=d.seu;h=e+a[6];break;case 'neu':g=d.neu,h=e+a[7]}f.push({children:b(g),numPoints:g.n,
name:h});c(g,h,f)}})};if(0===this.level%this.pcoGeometry.hierarchyStepSize){var e=this.level+this.pcoGeometry.baseDepth,d=this.greyhoundBounds,f=''+this.pcoGeometry.serverURL+'hierarchy?bounds=['+(d.min.x+','+d.min.y+','+d.min.z+','+d.max.x+','+d.max.y+','+d.max.z)+']&depthBegin='+e+'&depthEnd='+(e+this.pcoGeometry.hierarchyStepSize+2);this.scale&&(f+='&scale='+this.scale);this.greyhoundOffset&&(e=this.greyhoundOffset,f+='&offset=['+e.x+','+e.y+','+e.z+']');var g=this,k=new XMLHttpRequest;k.overrideMimeType('text/plain');
k.open('GET',f,!0);k.onload=function(a){try{var b=JSON.parse(k.responseText)||{};a=[];g.numPoints=b.n;c(b,g.name,a);b={};b[g.name]=g;for(var d=g.pcoGeometry,e=0;e<a.length;e++){var f=a[e].name,h=a[e].numPoints,w=parseInt(f.charAt(f.length-1)),r=f.substring(0,f.length-1),v=b[r],t=f.length-1,x=N.createChildAABB(v.boundingBox,w),y=new z(f,d,x,g.scale,g.offset);y.level=t;y.numPoints=h;y.hasChildren=0<a[e].children;y.spacing=d.spacing/Math.pow(2,t);v.addChild(y);b[f]=y}g.loadPoints()}catch(Ka){u.numNodesLoading--,
console.error('Potree: Exception thrown parsing points.',Ka)}};k.onerror=function(a){console.log('Potree: Failed to load file! HTTP status '+k.status+', file:'+f,a)};k.send(null)}};z.prototype.getNumPoints=function(){return this.numPoints};z.prototype.dispose=function(){if(this.geometry&&null!=this.parent){this.geometry.dispose();this.geometry=null;this.loaded=!1;for(var a=0;a<this.oneTimeDisposeHandlers.length;a++)(0,this.oneTimeDisposeHandlers[a])();this.oneTimeDisposeHandlers=[]}};Object.assign(z.prototype,
THREE.EventDispatcher.prototype);E.prototype.newerThan=function(a){a=new E(a);return this.versionMajor>a.versionMajor||this.versionMajor===a.versionMajor&&this.versionMinor>a.versionMinor?!0:!1};E.prototype.equalOrHigher=function(a){a=new E(a);return this.versionMajor>a.versionMajor||this.versionMajor===a.versionMajor&&this.versionMinor>=a.versionMinor?!0:!1};E.prototype.upTo=function(a){return!this.newerThan(a)};class ta{constructor(a,b,c){this.version='string'===typeof a?new E(a):a;this.boundingBox=
...
4,4)).getUint32(0,!0),e=a.pcoGeometry.pointAttributes;a.numPoints=c;c=a.boundingBox;var d=new THREE.Vector3;d=a.pcoGeometry.boundingBox.center(d).sub(a.boundingBox.min);b={buffer:b,pointAttributes:e,version:this.version.version,schema:a.pcoGeometry.schema,min:[c.min.x,c.min.y,c.min.z],max:[c.max.x,c.max.y,c.max.z],offset:d.toArray(),scale:this.scale,normalize:a.pcoGeometry.normalize};u.workerPool.runTask(y.GREYHOUND,function(b){b=b.data;var c=b.attributeBuffers,d=new THREE.Box3((new THREE.Vector3).fromArray(b.tightBoundingBox.min),
(new THREE.Vector3).fromArray(b.tightBoundingBox.max)),e=new THREE.BufferGeometry;e.isPoints=!0;for(var f in c){var m=c[f].buffer;parseInt(f)===x.POSITION_CARTESIAN?e.addAttribute('position',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(f)===x.COLOR_PACKED?e.addAttribute('color',new THREE.BufferAttribute(u.denormalizeUint8Array(new Uint8Array(m)),4)):parseInt(f)===x.INTENSITY?e.addAttribute('intensity',new THREE.BufferAttribute(new Float32Array(m),1)):parseInt(f)===x.CLASSIFICATION?e.addAttribute('classification',
new THREE.BufferAttribute(new Uint8Array(m),1)):parseInt(f)===x.NORMAL_SPHEREMAPPED?e.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(f)===x.NORMAL_OCT16?e.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(f)===x.NORMAL?e.addAttribute('normal',new THREE.BufferAttribute(new Float32Array(m),3)):parseInt(f)===x.INDICES?(m=new THREE.BufferAttribute(new Uint8Array(m),4),m.normalized=!0,e.addAttribute('indices',m)):parseInt(f)===x.SPACING&&
(m=new THREE.BufferAttribute(new Float32Array(m),1),e.addAttribute('spacing',m))}d.max.sub(d.min);d.min.set(0,0,0);a.numPoints=b.numPoints;a.geometry=e;a.mean=new THREE.Vector3(...b.mean);a.tightBoundingBox=d;a.loaded=!0;a.loading=!1;u.numNodesLoading--},b,[b.buffer])}}class H{static getQueryParam(a){a=a.replace(/[[\]]/g,'\\$&');return(a=(new RegExp('[?&]'+a+'(=([^&#]*)|&|#|$)')).exec(window.location.href))?a[2]?decodeURIComponent(a[2].replace(/\+/g,' ')):'':null}static createSchema(a){var b=[{name:'X',
size:4,type:'signed'},{name:'Y',size:4,type:'signed'},{name:'Z',size:4,type:'signed'}];a.forEach(function(a){'COLOR_PACKED'===a?(b.push({name:'Red',size:2,type:'unsigned'}),b.push({name:'Green',size:2,type:'unsigned'}),b.push({name:'Blue',size:2,type:'unsigned'})):'INTENSITY'===a?b.push({name:'Intensity',size:2,type:'unsigned'}):'CLASSIFICATION'===a&&b.push({name:'Classification',size:1,type:'unsigned'})});return b}static fetch(a,b){var c=new XMLHttpRequest;c.overrideMimeType('text/plain');c.open('GET',
a,!0);c.onreadystatechange=function(){4===c.readyState&&(200===c.status||0===c.status?b(null,c.responseText):b(c.responseText))};c.send(null)}static fetchBinary(a,b){var c=new XMLHttpRequest;c.overrideMimeType('text/plain');c.open('GET',a,!0);c.responseType='arraybuffer';c.onreadystatechange=function(){4===c.readyState&&(200===c.status||0===c.status?b(null,c.response):b(c.responseText))};c.send(null)}static pointSizeFrom(a){return a.reduce((a,c)=>a+c.size,0)}static getNormalization(a,b,c){var e=[{name:'X',
size:4,type:'floating'},{name:'Y',size:4,type:'floating'},{name:'Z',size:4,type:'floating'},{name:'Red',size:2,type:'unsigned'},{name:'Green',size:2,type:'unsigned'},{name:'Blue',size:2,type:'unsigned'},{name:'Intensity',size:2,type:'unsigned'}];a=a+'read?depth='+b+'&schema='+JSON.stringify(e);H.fetchBinary(a,function(a,b){if(a)throw Error(a);a=new DataView(b);b=b.byteLength-4;for(var d=H.pointSizeFrom(e),f=!1,h=!1,l=0;l<b;l+=d){if(255<a.getUint16(l+12,!0)||255<a.getUint16(l+14,!0)||255<a.getUint16(l+
16,!0))f=!0;255<a.getUint16(l+18,!0)&&(h=!0);if(f&&h)break}c(null,{color:f,intensity:h})})}}N.loadInfoJSON=function(a,b){};N.load=function(a,b){try{var c=a.split('greyhound://')[1];1===c.split('http://').length&&1===c.split('https://').length&&(c='http://'+c);H.fetch(c+'info',function(a,d){if(a)throw Error(a);a=JSON.parse(d);var e=new E('1.4'),g=a.bounds;d=a.scale||.01;Array.isArray(d)&&(d=Math.min(d[0],d[1],d[2]));H.getQueryParam('scale')&&(d=parseFloat(H.getQueryParam('scale')));var k=Math.max(8,
a.baseDepth),h=['POSITION_CARTESIAN'],l=!1,m=!1,p=!1;a.schema.forEach(function(a){'Intensity'===a.name&&h.push('INTENSITY');'Classification'===a.name&&h.push('CLASSIFICATION');'Red'===a.name?l=!0:'Green'===a.name?m=!0:'Blue'===a.name&&(p=!0)});l&&m&&p&&h.push('COLOR_PACKED');var n=new sa;n.serverURL=c;n.spacing=(g[3]-g[0])/Math.pow(2,k);n.baseDepth=k;n.hierarchyStepSize=5;n.schema=H.createSchema(h);k=H.pointSizeFrom(n.schema);n.pointAttributes=new F(h);n.pointAttributes.byteSize=k;g=new THREE.Box3((new THREE.Vector3).fromArray(g,
0),(new THREE.Vector3).fromArray(g,3));k=g.min.clone();g.max.sub(g.min);g.min.set(0,0,0);n.projection=a.srs;n.boundingBox=g;n.boundingSphere=g.getBoundingSphere(new THREE.Sphere);n.scale=d;n.offset=k;n.loader=new ta(e,g,n.scale);e={};d=new z('r',n,g,d,k);d.level=0;d.hasChildren=!0;d.numPoints=a.numPoints;d.spacing=n.spacing;n.root=d;n.root.load();e.r=d;n.nodes=e;H.getNormalization(c,a.baseDepth,function(a,c){c.color&&(n.normalize.color=!0);c.intensity&&(n.normalize.intensity=!0);b(n)})})}catch(e){console.log('Potree: Loading failed.',
a,e),b()}};N.loadPointAttributes=function(a){a=a.pointAttributes;for(var b=new F,c=0;c<a.length;c++)b.add(t[a[c]]);return b};N.createChildAABB=function(a,b){var c=a.min;a=a.max;var e=(new THREE.Vector3).copy(a).sub(c).multiplyScalar(.5),d=new THREE.Vector3(e.x,0,0),f=new THREE.Vector3(0,e.y,0),g=new THREE.Vector3(0,0,e.z),k=c,h=(new THREE.Vector3).add(c).add(e);1===b?(c=(new THREE.Vector3).copy(k).add(g),a=(new THREE.Vector3).copy(h).add(g)):3===b?(c=(new THREE.Vector3).copy(k).add(g).add(f),a=(new THREE.Vector3).copy(h).add(g).add(f)):
...
!1;throw a;});throw a;})})}handle(a,b){}}class Ma{constructor(a){this.node=a}push(a){var b=this,c={buffer:a.arrayb,numPoints:a.pointsCount,pointSize:a.pointSize,pointFormatID:2,scale:a.scale,offset:a.offset,mins:a.mins,maxs:a.maxs},e=u.workerPool.getWorker(y.LAS_DECODER);e.onmessage=function(c){var d=new THREE.BufferGeometry;d.isPoints=!0;var g=a.pointsCount,k=new Float32Array(c.data.position),h=new Uint8Array(c.data.color),l=new Float32Array(c.data.intensity),m=new Uint8Array(c.data.classification),
p=new Uint8Array(c.data.returnNumber),n=new Uint8Array(c.data.numberOfReturns),q=new Uint16Array(c.data.pointSourceID),w=new Uint8Array(c.data.indices);d.addAttribute('position',new THREE.BufferAttribute(k,3));d.addAttribute('color',new THREE.BufferAttribute(u.denormalizeUint8Array(h),4));d.addAttribute('intensity',new THREE.BufferAttribute(l,1));d.addAttribute('classification',new THREE.BufferAttribute(m,1));d.addAttribute('returnNumber',new THREE.BufferAttribute(p,1));d.addAttribute('numberOfReturns',
new THREE.BufferAttribute(n,1));d.addAttribute('pointSourceID',new THREE.BufferAttribute(q,1));d.addAttribute('indices',new THREE.BufferAttribute(w,4));d.attributes.indices.normalized=!0;k=new THREE.Box3((new THREE.Vector3).fromArray(c.data.tightBoundingBox.min),(new THREE.Vector3).fromArray(c.data.tightBoundingBox.max));d.boundingBox=b.node.boundingBox;b.node.tightBoundingBox=k;b.node.geometry=d;b.node.numPoints=g;b.node.loaded=!0;b.node.loading=!1;u.numNodesLoading--;b.node.mean=new THREE.Vector3(...c.data.mean);
u.workerPool.returnWorker(y.LAS_DECODER,e)};e.postMessage(c,[c.buffer])}}class wa{constructor(){this.octreeDir=this.url=null;this.spacing=0;this.pointAttributes=this.nodes=this.root=this.boundingBox=null;this.hierarchyStepSize=-1;this.loader=null}}class O extends V{constructor(a,b,c){super();this.id=O.IDCount++;this.name=a;this.index=parseInt(a.charAt(a.length-1));this.pcoGeometry=b;this.geometry=null;this.boundingBox=c;this.boundingSphere=c.getBoundingSphere(new THREE.Sphere);this.children={};this.numPoints=
0;this.level=null;this.loaded=!1;this.oneTimeDisposeHandlers=[]}isGeometryNode(){return!0}getLevel(){return this.level}isTreeNode(){return!1}isLoaded(){return this.loaded}getBoundingSphere(){return this.boundingSphere}getBoundingBox(){return this.boundingBox}getChildren(){for(var a=[],b=0;8>b;b++)this.children[b]&&a.push(this.children[b]);return a}getURL(){var a='',b=this.pcoGeometry.loader.version;b.equalOrHigher('1.5')?a=this.pcoGeometry.octreeDir+'/'+this.getHierarchyPath()+'/'+this.name:b.equalOrHigher('1.4')?
a=this.pcoGeometry.octreeDir+'/'+this.name:b.upTo('1.3')&&(a=this.pcoGeometry.octreeDir+'/'+this.name);return a}getHierarchyPath(){for(var a='r/',b=this.pcoGeometry.hierarchyStepSize,c=this.name.substr(1),e=Math.floor(c.length/b),d=0;d<e;d++)a+=c.substr(d*b,b)+'/';return a=a.slice(0,-1)}addChild(a){this.children[a.index]=a;a.parent=this}load(){if(!(!0===this.loading||!0===this.loaded||u.numNodesLoading>=u.maxNodesLoading)){this.loading=!0;u.numNodesLoading++;try{this.pcoGeometry.loader.version.equalOrHigher('1.5')?
...
c.open('GET',b,!0);c.responseType='arraybuffer';c.overrideMimeType('text/plain; charset=x-user-defined');c.onreadystatechange=()=>{4===c.readyState&&(200===c.status?this.parse(a,c.response):console.log('Failed '+b+': '+c.status))};try{c.send(null)}catch(e){console.log('Failed request: '+e)}}}parse(a,b){var c=u.workerPool.getWorker(y.EPT_BINARY_DECODER);c.onmessage=function(b){var d=new THREE.BufferGeometry;d.isPoints=!0;var e=b.data.numPoints,g=new Float32Array(b.data.position);d.addAttribute('position',
new THREE.BufferAttribute(g,3));g=new Uint8Array(b.data.indices);d.addAttribute('indices',new THREE.BufferAttribute(g,4));b.data.color&&(g=new Uint8Array(b.data.color),d.addAttribute('color',new THREE.BufferAttribute(u.denormalizeUint8Array(g),4)));b.data.intensity&&(g=new Float32Array(b.data.intensity),d.addAttribute('intensity',new THREE.BufferAttribute(g,1)));b.data.classification&&(g=new Uint8Array(b.data.classification),d.addAttribute('classification',new THREE.BufferAttribute(g,1)));b.data.returnNumber&&
(g=new Uint8Array(b.data.returnNumber),d.addAttribute('returnNumber',new THREE.BufferAttribute(g,1)));b.data.numberOfReturns&&(g=new Uint8Array(b.data.numberOfReturns),d.addAttribute('numberOfReturns',new THREE.BufferAttribute(g,1)));b.data.pointSourceId&&(g=new Uint16Array(b.data.pointSourceId),d.addAttribute('pointSourceID',new THREE.BufferAttribute(g,1)));d.attributes.indices.normalized=!0;g=new THREE.Box3((new THREE.Vector3).fromArray(b.data.tightBoundingBox.min),(new THREE.Vector3).fromArray(b.data.tightBoundingBox.max));
a.doneLoading(d,g,e,new THREE.Vector3(...b.data.mean));u.workerPool.returnWorker(y.EPT_BINARY_DECODER,c)};b={buffer:b,schema:a.ept.schema,scale:a.ept.eptScale,offset:a.ept.eptOffset,mins:(a=>[a.x,a.y,a.z])(a.key.b.min)};c.postMessage(b,[b.buffer])}}class ya{load(a){if(!a.loaded){var b=a.url()+'.laz',c=new XMLHttpRequest;c.open('GET',b,!0);c.responseType='arraybuffer';c.overrideMimeType('text/plain; charset=x-user-defined');c.onreadystatechange=()=>{4===c.readyState&&(200===c.status?this.parse(a,c.response):
console.log('Failed '+b+': '+c.status))};c.send(null)}}parse(a,b){var c=new J(b),e=new Na(a);c.open().then(()=>{c.isOpen=!0;return c.getHeader()}).then(b=>{var d=0,g=a=>[a.x,a.y,a.z],k=g(a.key.b.min),h=g(a.key.b.max),l=()=>c.readData(1E6,0,1).then(function(a){var f=new da(a.buffer,b.pointsFormatId,b.pointsStructSize,a.count,b.scale,b.offset,k,h);f.extraBytes=b.extraBytes;f.pointsFormatId=b.pointsFormatId;e.push(f);d+=a.count;if(a.hasMoreData)return l();b.totalRead=d;b.versionAsString=c.versionAsString;
b.isCompressed=c.isCompressed;return null});return l()}).then(()=>c.close()).then(()=>c.isOpen=!1).catch(a=>{console.log('Error reading LAZ:',a);if(c.isOpen)c.close().then(()=>{c.isOpen=!1;throw a;});else throw a;})}}class Na{constructor(a){this.node=a}push(a){var b=u.workerPool.getWorker(y.EPT_LAS_ZIP_DECODER);b.onmessage=c=>{var d=new THREE.BufferGeometry;d.isPoints=!0;var e=a.pointsCount,g=new Float32Array(c.data.position),k=new Uint8Array(c.data.color),h=new Float32Array(c.data.intensity),l=new Uint8Array(c.data.classification),
m=new Uint8Array(c.data.returnNumber),p=new Uint8Array(c.data.numberOfReturns),n=new Uint16Array(c.data.pointSourceID),q=new Uint8Array(c.data.indices);d.addAttribute('position',new THREE.BufferAttribute(g,3));d.addAttribute('color',new THREE.BufferAttribute(u.denormalizeUint8Array(k),4));d.addAttribute('intensity',new THREE.BufferAttribute(h,1));d.addAttribute('classification',new THREE.BufferAttribute(l,1));d.addAttribute('returnNumber',new THREE.BufferAttribute(m,1));d.addAttribute('numberOfReturns',
new THREE.BufferAttribute(p,1));d.addAttribute('pointSourceID',new THREE.BufferAttribute(n,1));d.addAttribute('indices',new THREE.BufferAttribute(q,4));d.attributes.indices.normalized=!0;g=new THREE.Box3((new THREE.Vector3).fromArray(c.data.tightBoundingBox.min),(new THREE.Vector3).fromArray(c.data.tightBoundingBox.max));this.node.doneLoading(d,g,e,new THREE.Vector3(...c.data.mean));u.workerPool.returnWorker(y.EPT_LAS_ZIP_DECODER,b)};var c={buffer:a.arrayb,numPoints:a.pointsCount,pointSize:a.pointSize,
pointFormatID:a.pointsFormatId,scale:a.scale,offset:a.offset,mins:a.mins,maxs:a.maxs};b.postMessage(c,[c.buffer])}}class D{static toVector3(a,b){return(new THREE.Vector3).fromArray(a,b||0)}static toBox3(a){return new THREE.Box3(D.toVector3(a),D.toVector3(a,3))}static findDim(a,b){a=a.find(a=>a.name==b);if(!a)throw Error('Failed to find '+b+' in schema');return a}static sphereFrom(a){return a.getBoundingSphere(new THREE.Sphere)}}class za{constructor(a,b){let c=b.schema,e=b.bounds,d=b.boundsConforming;
var f=[D.findDim(c,'X'),D.findDim(c,'Y'),D.findDim(c,'Z')];let g=f.map(a=>a.scale||1);f=f.map(a=>a.offset||0);this.eptScale=D.toVector3(g);this.eptOffset=D.toVector3(f);this.url=a;this.info=b;this.type='ept';this.schema=c;this.span=b.span||b.ticks;this.boundingBox=D.toBox3(e);this.tightBoundingBox=D.toBox3(d);this.offset=D.toVector3([0,0,0]);this.boundingSphere=D.sphereFrom(this.boundingBox);this.tightBoundingSphere=D.sphereFrom(this.tightBoundingBox);this.version=new E('1.6');this.fallbackProjection=
this.projection=null;b.srs&&b.srs.horizontal&&(this.projection=b.srs.authority+':'+b.srs.horizontal);b.srs.wkt&&(this.projection?this.fallbackProjection=b.srs.wkt:this.projection=b.srs.wkt);this.pointAttributes='LAZ';this.spacing=(this.boundingBox.max.x-this.boundingBox.min.x)/this.span;this.loader='binary'==(b.dataType||'laszip')?new xa:new ya}}class Aa{constructor(a,b,c,e,d,f){this.ept=a;this.b=b;this.d=c;this.x=e||0;this.y=d||0;this.z=f||0}name(){return this.d+'-'+this.x+'-'+this.y+'-'+this.z}step(a,
b,c){let e=this.b.min.clone(),d=this.b.max.clone(),f=(new THREE.Vector3).subVectors(d,e);a?e.x+=f.x/2:d.x-=f.x/2;b?e.y+=f.y/2:d.y-=f.y/2;c?e.z+=f.z/2:d.z-=f.z/2;return new Aa(this.ept,new THREE.Box3(e,d),this.d+1,2*this.x+a,2*this.y+b,2*this.z+c)}children(){for(var a=[],b=0;2>b;++b)for(var c=0;2>c;++c)for(var e=0;2>e;++e){var d=this.step(b,c,e).name();a.includes(d)||(a=a.concat(d))}return a}}class aa extends V{constructor(a,b,c,e,d,f){super();this.ept=a;this.key=new Aa(this.ept,b||this.ept.boundingBox,
c||0,e,d,f);this.id=aa.IDCount++;this.geometry=null;this.tightBoundingBox=this.boundingBox=this.key.b;this.spacing=this.ept.spacing/Math.pow(2,this.key.d);this.boundingSphere=D.sphereFrom(this.boundingBox);this.hasChildren=!1;this.children={};this.numPoints=-1;this.level=this.key.d;this.loading=this.loaded=!1;this.oneTimeDisposeHandlers=[];a=this.key;this.name=this.toPotreeName(a.d,a.x,a.y,a.z);this.index=parseInt(this.name.charAt(this.name.length-1))}isGeometryNode(){return!0}getLevel(){return this.level}isTreeNode(){return!1}isLoaded(){return this.loaded}getBoundingSphere(){return this.boundingSphere}getBoundingBox(){return this.boundingBox}url(){return this.ept.url+
...
h.spacing=(l.x+l.y+l.z)/3/75;h.estimatedSpacing=h.spacing;d.push(h);if(h.isLeaf)for(h=!1;!h&&0<d.length;)d.pop(),h=d[d.length-1],h=0<d.length&&h.hasRight&&null==h.right}this.root=f;this.levels=g;this.dispatchEvent({type:'hierarchy_loaded'})}};b.send(null)}get spacing(){if(this._spacing)return this._spacing;if(this.root)return this.root.spacing}set spacing(a){this._spacing=a}}ea.prototype={push:function(a){this.content.push(a);this.bubbleUp(this.content.length-1)},pop:function(){var a=this.content[0],
...
d;a=e}},sinkDown:function(a){for(var b=this.content.length,c=this.content[a],e=this.scoreFunction(c);;){var d=2*(a+1),f=d-1,g=null;if(f<b){var k=this.scoreFunction(this.content[f]);k<e&&(g=f)}d<b&&this.scoreFunction(this.content[d])<(null==g?e:k)&&(g=d);if(null==g)break;this.content[a]=this.content[g];this.content[g]=c;a=g}}};var Y={position:0,color:1,intensity:2,classification:3,returnNumber:4,numberOfReturns:5,pointSourceID:6,indices:7,normal:8,spacing:9},Da={DEFAULT:{0:new THREE.Vector4(.5,.5,
.5,1),1:new THREE.Vector4(.5,.5,.5,1),2:new THREE.Vector4(.63,.32,.18,1),3:new THREE.Vector4(0,1,0,1),4:new THREE.Vector4(0,.8,0,1),5:new THREE.Vector4(0,.6,0,1),6:new THREE.Vector4(1,.66,0,1),7:new THREE.Vector4(1,0,1,1),8:new THREE.Vector4(1,0,0,1),9:new THREE.Vector4(0,0,1,1),12:new THREE.Vector4(1,1,0,1),DEFAULT:new THREE.Vector4(.3,.6,.6,.5)}},T={NONE:0,HIGHLIGHT:1,SHOW_INSIDE:2,SHOW_OUTSIDE:3},Q={FIXED:0,ATTENUATED:1,ADAPTIVE:2},X={SQUARE:0,CIRCLE:1,PARABOLOID:2},A={RGB:0,COLOR:1,DEPTH:2,HEIGHT:3,
ELEVATION:3,INTENSITY:4,INTENSITY_GRADIENT:5,LOD:6,LEVEL_OF_DETAIL:6,POINT_INDEX:7,CLASSIFICATION:8,RETURN_NUMBER:9,SOURCE:10,NORMAL:11,PHONG:12,RGB_HEIGHT:13,COMPOSITE:50},W={OCTREE:0,KDTREE:1};class Qa{constructor(){this.boundingBox=new THREE.Box3;this.numPoints=0;this.data={}}add(a){var b=this.numPoints,c=b+a.numPoints,e=Object.keys(this.data),d=Object.keys(a.data),f=new Set([...e,...d]),g;for(g of f)if(e.includes(g)&&d.includes(g)){var k=this.data[g].constructor;f=new k(this.data[g].length+a.data[g].length);
...
b[0],b[1],b[2])}setUniform(a,b){b.constructor===THREE.Matrix4?this.setUniformMatrix4(a,b):'number'===typeof b?this.setUniform1f(a,b):'boolean'===typeof b?this.setUniformBoolean(a,b):b instanceof ja?this.setUniformTexture(a,b):b instanceof Array?2===b.length?this.setUniform2f(a,b):3===b.length&&this.setUniform3f(a,b):console.error('Potree: Unhandled uniform type: ',a,b)}setUniform1i(a,b){a=this.uniformLocations[a];null!=a&&this.gl.uniform1i(a,b)}}class Ga{constructor(){this.numElements=0;this.vao=
null;this.vbos=new Map}}class Ha extends THREE.Mesh{constructor(){super(new THREE.Geometry,new THREE.MeshBasicMaterial({opacity:0,wireframe:!1,transparent:!0}));this.rotation.set(-Math.PI/2,0,0);this.frustumCulled=!0;this.pointclouds=[];this.nodeSize=30;this.pointBudget=1E10;this.nodeLoadRate=2}raycast(a,b){}setPointBudget(a){this.pointBudget=a}onBeforeRender(a,b,c,e,d,f){for(b=0;b<this.pointclouds.length;b++)this.pointclouds[b].minimumNodePixelSize=this.nodeSize;na(this.pointclouds,c,a)}recalculateBoxGeometry(){var a=
this.getBoundingBox(),b=a.size(new THREE.Vector3),c=a.center(new THREE.Vector3);a=new THREE.Matrix4;a.makeTranslation(c.x,-c.z,c.y);b=(new THREE.BufferGeometry).fromGeometry(new THREE.BoxGeometry(b.x,b.z,b.y));b.applyMatrix(a);this.geometry=b}add(a){THREE.Object3D.prototype.add.call(this,a);a instanceof S&&(a.showBoundingBox=!1,a.generateDEM=!1,this.pointclouds.push(a),this.recalculateBoxGeometry())}remove(a){THREE.Object3D.prototype.remove.call(this,a);a instanceof S&&(a=this.pointclouds.indexOf(a),
-1!==a&&(this.pointclouds.splice(a,1),this.recalculateBoxGeometry()))}getBoundingBox(){var a=new THREE.Box3;this.updateMatrixWorld(!0);for(var b=0;b<this.pointclouds.length;b++){var c=this.pointclouds[b];c.updateMatrixWorld(!0);c=L.computeTransformedBoundingBox(c.pcoGeometry.tightBoundingBox?c.pcoGeometry.tightBoundingBox:c.boundingBox,c.matrixWorld);a.union(c)}return a}estimateHeightAt(a){var b=null,c=Infinity,e;for(e of this.pointclouds)if(void 0!==e.root.geometryNode){var d=null,f=Infinity,g=a.clone().sub(e.position);
g.z=0;g=new THREE.Ray(g,new THREE.Vector3(0,0,1));for(var k=[e.root];0<k.length;){var h=k.pop(),l=h.getBoundingBox();if(g.intersectBox(l)){l=h.geometryNode.mean.z+e.position.z+h.geometryNode.boundingBox.min.z;h.geometryNode.spacing<=f&&(d=l,f=h.geometryNode.spacing);for(var m of Object.keys(h.children))h.children[m].geometryNode&&k.push(h.children[m])}}if(null===b||f<c)b=d,c=f}return b}}class Ra extends Ha{constructor(){super();this.buffers=new Map;this.shaders=new Map;this.textures=new Map;this.types=
new Map}getExtensions(a){this.types.set(Float32Array,a.FLOAT);this.types.set(Uint8Array,a.UNSIGNED_BYTE);this.types.set(Uint16Array,a.UNSIGNED_SHORT);var b=a.getExtension('OES_vertex_array_object');a.createVertexArray=b.createVertexArrayOES.bind(b);a.bindVertexArray=b.bindVertexArrayOES.bind(b)}onBeforeRender(a,b,c,e,d,f){super.onBeforeRender(a,b,c,e,d,f);b=a.getContext();void 0===b.bindVertexArray&&this.getExtensions(b);e=this.fetchOctrees();for(var g of e.octrees)this.renderOctree(a,g,g.visibleNodes,
c);b.activeTexture(b.TEXTURE1);b.bindTexture(b.TEXTURE_2D,null);a.state.reset()}createBuffer(a,b){var c=new Ga;c.vao=a.createVertexArray();c.numElements=b.attributes.position.count;a.bindVertexArray(c.vao);for(var e in b.attributes){var d=b.attributes[e],f=a.createBuffer();a.bindBuffer(a.ARRAY_BUFFER,f);a.bufferData(a.ARRAY_BUFFER,d.array,a.STATIC_DRAW);var g=Y[e],k=d.normalized,h=this.types.get(d.array.constructor);void 0!==h&&(a.vertexAttribPointer(g,d.itemSize,h,k,0,0),a.enableVertexAttribArray(g));
c.vbos.set(e,{handle:f,name:e,count:d.count,itemSize:d.itemSize,type:b.attributes.position.array.constructor,version:0})}a.bindBuffer(a.ARRAY_BUFFER,null);a.bindVertexArray(null);return c}updateBuffer(a,b){var c=this.buffers.get(b);a.bindVertexArray(c.vao);for(var e in b.attributes){var d=b.attributes[e],f=Y[e],g=d.normalized,k=this.types.get(d.array.constructor);if(c.vbos.has(e)){var h=c.vbos.get(e).handle;c.vbos.get(e).version=d.version}else h=a.createBuffer(),c.vbos.set(e,{handle:h,name:e,count:d.count,
itemSize:d.itemSize,type:b.attributes.position.array.constructor,version:d.version});a.bindBuffer(a.ARRAY_BUFFER,h);a.bufferData(a.ARRAY_BUFFER,d.array,a.STATIC_DRAW);a.vertexAttribPointer(f,d.itemSize,k,g,0,0);a.enableVertexAttribArray(f)}a.bindBuffer(a.ARRAY_BUFFER,null);a.bindVertexArray(null)}fetchOctrees(){for(var a=[],b=[this];0<b.length;){var c=b.pop();c instanceof S?a.push(c):(c=c.children.filter(a=>a.visible),b.push(...c))}return{octrees:a}}renderNodes(a,b,c,e,d,f){a=a.getContext();b=b.material;
d=d.matrixWorldInverse;var g=new THREE.Matrix4,k=new Float32Array(16),h;for(h of c)if(void 0===u.debug.allowedNodes||u.debug.allowedNodes.includes(h.name)){var l=h.sceneNode.matrixWorld;g.multiplyMatrices(d,l);e&&(c=e.offsets.get(h),f.setUniform1f('uVNStart',c));c=h.getLevel();f.setUniform('uDebug',!0===h.debug);if(h instanceof ba)var m=0===Object.keys(h.children).length;else h instanceof ha&&(m=h.geometryNode.isLeaf);f.setUniform('uIsLeafNode',m);var p=f.uniformLocations.modelMatrix;p&&(k.set(l.elements),
a.uniformMatrix4fv(p,!1,k));p=f.uniformLocations.modelViewMatrix;k.set(g.elements);a.uniformMatrix4fv(p,!1,k);if(b.clipPolygons&&0<b.clipPolygons.length){p=[];var n=[];for(r of b.clipPolygons){d=r.viewMatrix;var q=r.projMatrix.clone().multiply(d).multiply(l);p.push(r.markers.length);n.push(q)}l=[].concat(...n.map(a=>a.elements));q=Array(24*b.clipPolygons.length);for(n=0;n<b.clipPolygons.length;n++){var r=b.clipPolygons[n];for(var t=0;t<r.markers.length;t++)q[24*n+3*t]=r.markers[t].position.x,q[24*
n+(3*t+1)]=r.markers[t].position.y,q[24*n+(3*t+2)]=r.markers[t].position.z}a.uniform1iv(f.uniformLocations['uClipPolygonVCount[0]'],p);a.uniformMatrix4fv(f.uniformLocations['uClipPolygonWVP[0]'],!1,l);a.uniform3fv(f.uniformLocations['uClipPolygonVertices[0]'],q)}f.setUniform1f('uLevel',c);f.setUniform1f('uNodeSpacing',h.geometryNode.estimatedSpacing);f.setUniform1f('uPCIndex',n);c=h.geometryNode.geometry;l=null;if(this.buffers.has(c)){l=this.buffers.get(c);for(var v in c.attributes)c.attributes[v].version>
l.vbos.get(v).version&&this.updateBuffer(a,c)}else l=this.createBuffer(a,c),this.buffers.set(c,l);a.bindVertexArray(l.vao);a.drawArrays(a.POINTS,0,l.numElements)}a.bindVertexArray(null)}renderOctree(a,b,c,e){var d=a.getContext(),f=b.material,g=e.matrixWorldInverse,k=e.matrixWorld,h=e.projectionMatrix,l=(new THREE.Matrix4).getInverse(h);new THREE.Matrix4;var m=null,p=0;if(f.pointSizeType===Q.ADAPTIVE||f.pointColorType===A.LOD){m=b.computeVisibilityTextureData(c,e);var n=f.visibleNodesTexture;n.image.data.set(m.data);
