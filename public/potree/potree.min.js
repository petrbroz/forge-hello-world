...
d.addAttribute('pointSourceID',new THREE.BufferAttribute(q,1));d.addAttribute('indices',new THREE.BufferAttribute(w,4));d.attributes.indices.normalized=!0;k=new THREE.Box3((new THREE.Vector3).fromArray(c.data.tightBoundingBox.min),(new THREE.Vector3).fromArray(c.data.tightBoundingBox.max));d.boundingBox=b.node.boundingBox;b.node.tightBoundingBox=k;b.node.geometry=d;b.node.numPoints=g;b.node.loaded=!0;b.node.loading=!1;u.numNodesLoading--;b.node.mean=new THREE.Vector3(...c.data.mean);u.workerPool.returnWorker(y.LAS_DECODER,
e)};e.postMessage(c,[c.buffer])}}class wa{constructor(){this.octreeDir=this.url=null;this.spacing=0;this.pointAttributes=this.nodes=this.root=this.boundingBox=null;this.hierarchyStepSize=-1;this.loader=null}}class O extends V{constructor(a,b,c){super();this.id=O.IDCount++;this.name=a;this.index=parseInt(a.charAt(a.length-1));this.pcoGeometry=b;this.geometry=null;this.boundingBox=c;this.boundingSphere=c.getBoundingSphere(new THREE.Sphere);this.children={};this.numPoints=0;this.level=null;this.loaded=
!1;this.oneTimeDisposeHandlers=[]}isGeometryNode(){return!0}getLevel(){return this.level}isTreeNode(){return!1}isLoaded(){return this.loaded}getBoundingSphere(){return this.boundingSphere}getBoundingBox(){return this.boundingBox}getChildren(){for(var a=[],b=0;8>b;b++)this.children[b]&&a.push(this.children[b]);return a}getURL(){var a='',b=this.pcoGeometry.loader.version;b.equalOrHigher('1.5')?a=this.pcoGeometry.octreeDir+'/'+this.getHierarchyPath()+'/'+this.name:b.equalOrHigher('1.4')?a=this.pcoGeometry.octreeDir+
...
n=new Uint16Array(c.data.pointSourceID),q=new Uint8Array(c.data.indices);d.addAttribute('position',new THREE.BufferAttribute(g,3));d.addAttribute('color',new THREE.BufferAttribute(u.denormalizeUint8Array(k),4));d.addAttribute('intensity',new THREE.BufferAttribute(h,1));d.addAttribute('classification',new THREE.BufferAttribute(l,1));d.addAttribute('returnNumber',new THREE.BufferAttribute(m,1));d.addAttribute('numberOfReturns',new THREE.BufferAttribute(p,1));d.addAttribute('pointSourceID',new THREE.BufferAttribute(n,
1));d.addAttribute('indices',new THREE.BufferAttribute(q,4));d.attributes.indices.normalized=!0;g=new THREE.Box3((new THREE.Vector3).fromArray(c.data.tightBoundingBox.min),(new THREE.Vector3).fromArray(c.data.tightBoundingBox.max));this.node.doneLoading(d,g,e,new THREE.Vector3(...c.data.mean));u.workerPool.returnWorker(y.EPT_LAS_ZIP_DECODER,b)};var c={buffer:a.arrayb,numPoints:a.pointsCount,pointSize:a.pointSize,pointFormatID:a.pointsFormatId,scale:a.scale,offset:a.offset,mins:a.mins,maxs:a.maxs};
b.postMessage(c,[c.buffer])}}class D{static toVector3(a,b){return(new THREE.Vector3).fromArray(a,b||0)}static toBox3(a){return new THREE.Box3(D.toVector3(a),D.toVector3(a,3))}static findDim(a,b){a=a.find(a=>a.name==b);if(!a)throw Error('Failed to find '+b+' in schema');return a}static sphereFrom(a){return a.getBoundingSphere(new THREE.Sphere)}}class za{constructor(a,b){let c=b.schema,e=b.bounds,d=b.boundsConforming;var f=[D.findDim(c,'X'),D.findDim(c,'Y'),D.findDim(c,'Z')];let g=f.map(a=>a.scale||
1);f=f.map(a=>a.offset||0);this.eptScale=D.toVector3(g);this.eptOffset=D.toVector3(f);this.url=a;this.info=b;this.type='ept';this.schema=c;this.span=b.span||b.ticks;this.boundingBox=D.toBox3(e);this.tightBoundingBox=D.toBox3(d);this.offset=D.toVector3([0,0,0]);this.boundingSphere=D.sphereFrom(this.boundingBox);this.tightBoundingSphere=D.sphereFrom(this.tightBoundingBox);this.version=new E('1.6');this.fallbackProjection=this.projection=null;b.srs&&b.srs.horizontal&&(this.projection=b.srs.authority+
':'+b.srs.horizontal);b.srs.wkt&&(this.projection?this.fallbackProjection=b.srs.wkt:this.projection=b.srs.wkt);this.pointAttributes='LAZ';this.spacing=(this.boundingBox.max.x-this.boundingBox.min.x)/this.span;this.loader='binary'==(b.dataType||'laszip')?new xa:new ya}}class Aa{constructor(a,b,c,e,d,f){this.ept=a;this.b=b;this.d=c;this.x=e||0;this.y=d||0;this.z=f||0}name(){return this.d+'-'+this.x+'-'+this.y+'-'+this.z}step(a,b,c){let e=this.b.min.clone(),d=this.b.max.clone(),f=(new THREE.Vector3).subVectors(d,
e);a?e.x+=f.x/2:d.x-=f.x/2;b?e.y+=f.y/2:d.y-=f.y/2;c?e.z+=f.z/2:d.z-=f.z/2;return new Aa(this.ept,new THREE.Box3(e,d),this.d+1,2*this.x+a,2*this.y+b,2*this.z+c)}children(){for(var a=[],b=0;2>b;++b)for(var c=0;2>c;++c)for(var e=0;2>e;++e){var d=this.step(b,c,e).name();a.includes(d)||(a=a.concat(d))}return a}}class aa extends V{constructor(a,b,c,e,d,f){super();this.ept=a;this.key=new Aa(this.ept,b||this.ept.boundingBox,c||0,e,d,f);this.id=aa.IDCount++;this.geometry=null;this.tightBoundingBox=this.boundingBox=
